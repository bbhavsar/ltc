...
void twosum_noset(const vector<int>& in, int target, int l, vector< vector<int> >& result) {
    for (int r = in.size() - 1; l < r;) {
        if (in[l] + in[r] == target) {
...
void threesum(vector<int>& in, int target, vector< vector<int> >& result) {
    ...
    for (int l = 0; l < in.size(); l++) {
        ...
        twosum_noset(in, target - elem, l + 1, result);
...
void ksum_impl(const vector<int>& in, int target, int k, int l, vector< vector<int> >& result) {
    assert(k >= 2);
    if (k == 2) {
        return twosum_noset(in, target, l, result);
    }

    for (int idx = l; idx < in.size(); idx++) {
        int elem = in[idx];
        int size_before = result.size();
        ksum_impl(in, target - elem, k - 1, idx + 1, result);
        int size_after = result.size();
        int num_inserts = size_after - size_before;
        if (num_inserts == 0) {
            continue;
...
void ksum(vector<int>& in, int target, int k, vector< vector<int> >& result) {
    ...
    return ksum_impl(in, target, k, 0, result);
}
...
int main() {
    ...
    result.clear();
    twosum_noset(in, 12, 0, result);
    print_result(result);

    result.clear();
    threesum(in, 18, result);
    print_result(result);
