 ...
 * modify the heights of each tower either by increasing or decreasing them by K only
 * once and then find out the minimum difference of the heights of shortest and longest towers.
...
void min_tower_diff(const vector<int>& in, int idx, int k, vector<int>& curr, int& min_diff) {
    ...
    const vector<int> opts = {in[idx] + k, in[idx] - k};
    for (int elem : opts) {
        if (elem >= 0) {
            curr.push_back(elem);
